...
def get_support_from_bipartition(tree, clade, mult):
    ...
    clade2 = {c for c in tree.taxon_namespace} - set(clade)
...
def getTaxa(cladeFiles):
    ...
    for line in lines:
        ...
        if (tmp == "All"):
            ...
            taxa = set(listLine.replace('""+""', '\t').replace('""', '\t').replace('"', '\t').replace('+', '\t').strip(
                '\t').replace('\t\t', '\t').split('\t'))
    f.close()
    return taxa
...
def main(*arg):
    ...
    for fileName in arg[4:][0].split(' '):
        ...
        for i, treetmp in enumerate(trees):
            treeName = "%s_%s" % (fileName, i)
            tree = treetmp.clone(2)

            tree.write(path=outFile2,schema="newick",suppress_rooting=True, unquoted_underscores=True,preserve_spaces=True)
            tree = dendropy.Tree.get(path=outFile2, schema='newick', rooting="force-unrooted",preserve_underscores=True)
            labelsSet = set(t.label for t in tree.taxon_namespace)
...
if __name__ == '__main__':
    ...
    for fileName in sys.argv[4:]:
        ...
        for i, treetmp in enumerate(trees):
            treeName = "%s_%s" % (fileName, i)
            tree = treetmp.clone(2)
            tree = tree.purge_taxon_namespace()
